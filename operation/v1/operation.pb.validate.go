...
// Validate checks the field values on AuditAppealReques with the rules defined
// in the proto definition for this message. If any rules are violated, the
// first error encountered is returned, or nil if there are no violations.
func (m *AuditAppealReques) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on AuditAppealReques with the rules
// defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// AuditAppealRequesMultiError, or nil if none found.
func (m *AuditAppealReques) ValidateAll() error {
	return m.validate(true)
}

func (m *AuditAppealReques) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if m.GetAppealID() <= 0 {
		err := AuditAppealRequesValidationError{
			field:  "AppealID",
			reason: "value must be greater than 0",
		}
		if !all {
	...
	if len(errors) > 0 {
		return AuditAppealRequesMultiError(errors)
	}

	return nil
}

// AuditAppealRequesMultiError is an error wrapping multiple validation errors
// returned by AuditAppealReques.ValidateAll() if the designated constraints
// aren't met.
type AuditAppealRequesMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m AuditAppealRequesMultiError) Error() string {
	msgs := make([]string, 0, len(m))
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m AuditAppealRequesMultiError) AllErrors() []error { return m }

// AuditAppealRequesValidationError is the validation error returned by
// AuditAppealReques.Validate if the designated constraints aren't met.
type AuditAppealRequesValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e AuditAppealRequesValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e AuditAppealRequesValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e AuditAppealRequesValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e AuditAppealRequesValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e AuditAppealRequesValidationError) ErrorName() string {
	return "AuditAppealRequesValidationError"
}

// Error satisfies the builtin error interface
func (e AuditAppealRequesValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sAuditAppealReques.%s: %s%s",
		key,
		e.field,
		e.reason,
		cause)
...
var _ error = AuditAppealRequesValidationError{}
...
var _ interface {
	...
	Cause() error
	ErrorName() string
} = AuditAppealRequesValidationError{}
